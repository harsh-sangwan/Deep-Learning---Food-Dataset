...
class ImageUtils(object):
    ...
    def __init__(self):
        ...
        self.img_train_raw_file = None
        self.img_inventory_raw_file = None
    ...
    def configure(self, info):
        ...
        self.img_train_raw_file = info["img_train_raw_file"]
        self.img_inventory_raw_file = info["img_inventory_raw_file"]
    ...
    def raw2resized_load_save(self, raw_dir=None, processed_dir=None, img_shape=None):
        ...
        for i, raw_filename in enumerate(raw_filenames_list):
            ...
            processed_shortname = name.split('.')[0] + "_resized." + tag
    ...
    def raw2resizednorm_load(self, raw_dir=None, img_shape=None):
        ...
        # Extract filenames from dir
        raw_filenames_list, n_files = self.extract_filenames2(raw_dir, 1)
    ...
    """
     Read a raw directory of images, and load as numpy array to memory
    """
    def raw2resizednorm_load_classify(self, raw_dir=None, classes=None, img_shape=None):
        ...
        for i, raw_filename in enumerate(raw_filenames_list):
            ...
            img_raw = self.read_img(raw_filename, gray_scale=gray_scale)
            #print(raw_filename)
            y_label = raw_filename.split('/')[-1]
            y_label = y_label.split(':')[0]
            
            #print(y_label)
            y_label_int = int(classes.index(y_label))
        ...
        return img_list, y_labels, raw_filenames_list
    ...
    """
     ...
     random number generator
    """
    def split_train_test_classify(self, x_data, y_data, ratio_train_test, seed):
        ...
        for i in range(n):
            ...
            if r_list[i] < ratio_train_test:
                x_data_train.append(x_data[i])
                y_data_train.append(y_data[i])
                index_train.append(i)
            else:
                x_data_test.append(x_data[i])
                y_data_test.append(y_data[i])
                index_test.append(i)

        # Convert to numpy arrays
        x_data_train = np.array(x_data_train, dtype=float)
        x_data_test  = np.array(x_data_test, dtype=float)
        y_data_train = np.array(y_data_train, dtype=float)
        y_data_test  = np.array(y_data_test, dtype=float)
        index_train  = np.array(index_train, dtype=int)
        index_test   = np.array(index_test, dtype=int)
        ...
        return x_data_train, x_data_test, y_data_train, y_data_test, index_train, index_test
    ...
    def extract_name_tag(self, full_filename):
        tag = full_filename.split('.')[-1]
        name = full_filename.split('/')[2] + ':' + full_filename.split('/')[3]
        return name, tag

    def extract_name_tag2(self, full_filename):
    ...
    """
     Extract all filenames inside a given directory (with fraction option):
      - list of filenames
    """
    def extract_filenames(self, file_name, frac_take):
        filenames_list = []
        with open(file_name, 'r') as f:
            for line in f:
                line = line[:-1]
                img_name = 'food-101/images/'+line+'.jpg'
                filenames_list.append(img_name)
        n_files = len(filenames_list)
        return filenames_list, n_files

    def extract_filenames2(self, dir, frac_take):
