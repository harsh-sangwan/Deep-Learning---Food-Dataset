    ...
    for i in range(N):
      ...
      for j in range(Hout):
        for k in range(Wout):
          ...
          db += dout[i, :, j, k]
          dx[i:i + 1, :, j * stride:j * stride + HH, k * stride:k * stride + WW] += (w * dout[i, :, j, k].reshape(-1, 1, 1, 1)).sum(axis=0)
...
def batchnorm_backward(dout, cache):
  ...
  gamma        = cache[3]
  eps          = cache[8]
  x_minus_mean = cache[4]
  var          = cache[7]
  ivar         = cache[5]
  x_norm       = cache[1]

  dx_hat = dout * gamma 
  dsample_var = np.sum(dx_hat * x_minus_mean * (-0.5) * (var + eps)**(-1.5), axis=0)
  dsample_mean = np.sum(dx_hat * (-1 * ivar) , axis=0)# + dsample_var * ((np.sum(-2*x_minus_mean)) / m)

  dx = dx_hat * ivar + dsample_var * (2*x_minus_mean/m) + dsample_mean/m
